...
exports.run = function (command, target, soldiers) {
  ...
  if (command == 'get') {
    var page = soldiers || 1;
    if (target == 'public') {
      request('http://insekai.com/api/latest?limit=8&page=' + page, function (error, response, body) {
      ...
      fs.readFile(home + '/.sekaiconfig', function (err, data) {
        if (err) unconfig();
        var key = JSON.parse(data).key; 
        request('http://insekai.com/api/follow_timeline?limit=8&key=' + key + '&page=' + page , function (error, response, body) {
          if (!error && response.statusCode == 200) {
            var content = JSON.parse(body);
            for (var order in content) {
              console.log(content[order].username.yellow);
      ...
      request('http://insekai.com/api/' + target +'/timeline?limit=8&page=' + page, function (error, response, body) {
        if (!error && response.statusCode == 200) {
          var content = JSON.parse(body);
          for (var order in content) {
            console.log(content[order].username.yellow);
