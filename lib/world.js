      ...
      request('http://insekai.com/api/latest?limit=5&page=' + page, function (error, response, body) {
        if (!error && response.statusCode == 200) {
          ...
          for (var order in content) {
            console.log(content[order].username.yellow + ' ['.grey + content[order].tokio_id.grey + ']'.grey);
    ...
    fs.readFile(home + '/.sekaiconfig', function (err, data) {
        ...
        prompt.get(['reply'], function (err, result) {
          ...
          request.post({
            ...
            var result = JSON.parse(body);
            if (result.status == 'good') {
              console.log('Successfully replied to No.%d, see http://insekai.com/t/%d'.green, tokio_id, result.tokio_id);
            } else {
